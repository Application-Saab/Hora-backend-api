...
router.post("/get-presigned-url", async (req, res) => {
  try {
    const { fileName, fileType, folder = "venue-images", userId, venueId, category = "other" } = req.body;

    if (!fileName || !fileType || !userId || !venueId) {
      return res.status(400).json({ message: "Missing required fields" });
    }

    // Example key: venue-images/{userId}/{venueId}/{category}/{timestamp-filename}
    const key = `${folder}/${userId}/${venueId}/${category}/${Date.now()}-${fileName}`;
    ...
    const params = {
      ...
      // Optional: ACL: "public-read" (only if your bucket policy requires it)
    };
...
router.get("/venue-images/:venueId", async (req, res) => {
  try {
    ...
    const data = await VenueImages.findOne({
      venueId: new mongoose.Types.ObjectId(venueId),
    }).lean();
    ...
    console.error("Fetch venue images error:", err);
    res.status(500).json({ message: "Server error" });
  }
});
