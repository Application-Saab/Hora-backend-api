...
const { upload, uploadFileToS3, generateThumbnail } = require('../store/multerS3Config');
...
router.post('/CreateFolder', async (req, res) => {
    try {
      ...
      const existingFolder = await FolderModel.findOne({ folderName, customerId });
      if (existingFolder) {
        return res.status(400).json({ message: `Folder with the name '${folderName}' already exists for this customer.` });
      }
...
router.post('/DeleteFolder', async (req, res) => {
    try {
      ...
      if (!folder) {
        return res.status(404).json({ message: `Folder with the name '${folderName}' not found for this customer.` });
      }
...
router.post('/DeleteFolder', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `Folder '${folderName}' deleted successfully from both database and S3.` });
    } catch (error) {
        res.status(500).json({ message: 'Server error', error: error.message });
    }
});
...
router.post('/UpdateFolder', async (req, res) => {
    try {
      ...
      if (!newFolderName || !customerId) {
        return res.status(400).json({ message: 'New Folder Name and Customer ID are required' });
      }
      ...
      const existingFolder = await FolderModel.findOne({ folderName: newFolderName, customerId });
      if (existingFolder) {
        return res.status(400).json({ message: `Folder with the name '${newFolderName}' already exists for this customer.` });
      }
...
router.get('/GetFoldersByCustomerId/:customerId', async (req, res) => {
    try {
      ...
      if (folders.length === 0) {
        return res.status(404).json({ message: `No folders found for customer with ID '${customerId}'` });
      }
...
router.get('/thumbnailsWithinProject', async (req, res) => {
  try {
    ...
    const thumbFiles = s3Response.Contents.filter(file => file.Key.includes('/thumb_'));
